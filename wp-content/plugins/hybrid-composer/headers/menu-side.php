...
for ($i = 0; $i < count($menu_arr); $i++) {
    ...
    if ($menu_css[0] != "menu-obj") {
        if (count($menu_item[2]) == 0) $menu_html .= '<li ' . $menu_css[1] . '><a href="' . esc_url($menu_item[1]) . '">'. $menu_css[0] .  esc_attr($menu_item[0]) . '</a></li>';
            ...
            if ($is_mega) $menu_html .= '<li class="panel-item mega-item"><span>'. $menu_css[0] . $menu_item[0] . '<span class="scn-down sidebar-dropdown-arrow"></span>' . '</span>';
            else $menu_html .= '<li' . $menu_css[1] . '><a class="dropdown-toggle" data-toggle="dropdown" href="' .  esc_url($menu_item[1]) . '">' . $menu_css[0] . esc_attr($menu_item[0]) . '<span class="scn-down sidebar-dropdown-arrow"></span>' . '</a><ul class="collapse">';
...
<header class="<?php echo $classes ?>" <?php echo esc_attr($hc_menu_animation) ?>>
                ...
                <div class="nav navbar-nav navbar-right">
                    ...
                    <form role="search" method="get" id="searchform" onsubmit='<?php echo $home_url ?>' class="navbar-form" action="<?php echo $home_url ?>">
                        <div class="input-group">
                            <input name="s" id="s" type="text" class="form-control" placeholder="<?php esc_attr_e( 'Type and hit enter','hc'); ?>" />
                        </div>
                    </form>
                    <?php } ?>
    ...
    <?php  } else {
               echo '<a class="hamburger-button hamburger-mobile" data-menu-anima="fade-left"><span class="scn-menu"></span></a>';
          } ?>
