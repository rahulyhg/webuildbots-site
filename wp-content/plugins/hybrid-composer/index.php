...
function hc_after_setup_theme(){
    load_theme_textdomain( 'hc', HC_PLUGIN_PATH. '/languages' );
...
function hc_get_post_categories($post_id,$slug) {
    $cat_html = "";
    $post_categories;
    if ($slug != 'post') $post_categories = get_the_terms($post_id, $slug . "-cat");
    else $post_categories = wp_get_post_categories($post_id);
    if ($post_categories !== false && !is_wp_error($post_categories)) {
        foreach($post_categories as $c){
            $cat = get_category( $c );
            $cat_html .= '<a class="category-'. $cat->slug . '" href="'. get_category_link($cat->term_id) .'">'. $cat->name . '</a>, ';
        }
    }
...
function hc_archive_engine() {
    ...
    if (count($wp_query->tax_query->queries) > 0) {
        ...
        $txt = "tag ";
        if ($current_post_type != 'post') $txt = esc_attr__("category ","hc");
        $title = ucfirst($current_post_type) . esc_attr__(" archive with " . $txt,"hc") . $tag;
        $subtitle = esc_attr__("All " . $current_post_type . " items with " . $txt,"hc") . $tag;
    }
    if ($date != "") {
        $title = ucfirst($current_post_type) . esc_attr__(" archive with date ","hc") . $date;
        $subtitle = esc_attr__("All " . $current_post_type . " items with date ","hc") . $date;
    }
    if (count($wp_query->tax_query->queries) > 0) $HC_PT_CATEGORY = $wp_query->tax_query->queries[0]['terms'][0];
...
function hc_category_engine() {
    ...
    $HC_IS_ARCHIVE = true;
    if (count($wp_query->tax_query->queries) > 0) $HC_PT_CATEGORY = $wp_query->tax_query->queries[0]['terms'][0];
    $archive_post_id = hc_get_setting('blog-page');
    if ($archive_post_id > 0) {
        $HC_PAGE_ARR = json_decode(get_post($archive_post_id)->post_content, true);
        if (key_exists('main-title',$HC_PAGE_ARR)) {
            $cat = ucfirst($wp_query->queried_object->name);
            if ($cat != "") {
                $HC_PAGE_ARR['main-title']['title'] =  $cat;
                $HC_PAGE_ARR['main-title']['subtitle'] = esc_attr__("Posts with category ","hc")  . $cat;
            }
        }
        get_header();
        hc_get_title();
...
<?php
        ...
        get_header();
        $cat = "";
        if (count($wp_query->tax_query->queries) > 0) $cat = $wp_query->queried_object->name;
?>
<div class="header-base no-subtitle">
    <div class="container">
        <div class="row">
            <div class="col-md-9">
                <div class="title-base text-left">
                    <h1>
                        <?php echo esc_attr_e("Posts with category: ","hc") . esc_attr($cat) ?>
                    </h1>
                    <p>
                        <?php echo esc_attr_e("Lists of posts with category","hc") . " <b>" . esc_attr($cat) . "</b>" ?>
                    </p>
                </div>
            </div>
        </div>
...
<?php }
    ...
    <div class="background-page overlay-container">
        <video autoplay loop muted poster="<?php echo hc_get_img($HC_PAGE_ARR["template_setting"]["template_video"]["image"],"hd"); ?>" class="background-fullscreen">
...
?>
    ...
    if ($author) hc_echo(ucfirst(get_the_author()),'<span><i class="fa fa-pencil"></i><a> ','</a></span>');
...
<footer class="default-wp-footer">
    <div class="content">
        <div class="container">
            <div><?php echo esc_attr__("Copyright ","wptf")?> <a href="<?php echo esc_url("http://socanny.com") ?>" target="_blank"><?php echo esc_attr("SoCanny")?></a><?php echo esc_attr__(". All Rights Reserved. Powered by","wptf") ?> <a href="<?php echo esc_url("http://wordpress.org") ?>" target="_blank"><?php echo esc_attr("WordPress")?></a><?php echo esc_attr__(".","wptf")?></div>
        </div>
    </div>
</footer>  
<?php } else { ?>
<?php if (hc_get_setting('scroll-top-btn') != "") echo '<span class="scroll-top scroll-top-mobile scn-up' . ((hc_get_setting('scroll-top-btn') =="always") ? " show" : "") . '"></span>'; ?>
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="<?php echo esc_attr($col_1) ?> footer-left">
                      <?php hc_echo(html_entity_decode($footer_content_1),"","") ?>
                </div>
                <?php if ($col_2 != "") { ?>
                <div class="<?php echo esc_attr($col_2) ?>">
                    <?php hc_echo(html_entity_decode($footer_content_2),"","") ?>
                ...
                <div class="<?php echo esc_attr($col_3) ?> ">
                    <?php 
                      hc_echo(html_entity_decode($footer_content_3),'','<span class="space"></span><span class="space"></span>'); 
