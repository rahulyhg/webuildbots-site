    ...
    $(document).ready(function () {
        ...
        if (body_class_arr.indexOf("post-type-page") > -1 || body_class_arr.indexOf("post-type-y-post-types") > -1 || $("#y-post-types-single-area").length || body_class_arr.indexOf("post-type-post") > -1) {
            ...
            if ($("#y-post-types-single-area").length) {
                if (!isPostBlog) $(body).addClass("y-post-types-single").addClass("hc-locked-mode");
            ...
            $(body).on("click", "div,input,span,li,a,i,th", function (e) {
                    ...
                    $(this).open_upload_box(function (target, attachment) {
                        if ($(cnt).hasClass("upload-multi")) {
                            ...
                            for (var i = 0; i < attachment.length; i++) {
                                ...
                                if (img_size[0] > img.width || img_size[0] > img.height) img_html = img_html.replace('class="upload-btn"', 'class="upload-btn small-bg"');
                                html += img_html;
                            }
                            $(cnt).append(html);
                        } else {
                ...
                if (css.indexOf("composer-item-copy") > -1) {
                    ...
                    if (!isEmpty($(clone).attr("data-hc-id"))) {
                        $(clone).attr("data-hc-id", makeID());
                    }
                    $(clone).find("[data-hc-id]").each(function () {
                        $(this).attr("data-hc-id", makeID());
                    });
                ...
                if (css.indexOf("composer-item-paste") > -1) {
                    ...
                    if (!isEmpty(html)) {
                        ...
                        var sec = "#hybrid_composer_content > .hc-cnt-content:last-child";
...
function makeID(count) {
    ...
    $("[data-hc-id]").each(function () {
        currentArr.push($(this).attr("data-hc-id"));
    });

    for (var i = 0; i < count; i++) {
        while (inArray(currentArr, text)) {
            text = "";
            for (var j = 0; j < 5; j++)
