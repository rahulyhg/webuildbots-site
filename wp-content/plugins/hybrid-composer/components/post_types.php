...
<div class="advs-box advs-box-side-icon <?php echo $css ?>" <?php if ($animation != "") echo 'data-anima="' . $animation . '" data-trigger="hover"'; ?>>
    <?php hc_echo(hc_get_icon($item["icon"]["icon"], $item["icon"]["icon_style"], $item["icon"]["icon_image"], "icon" . (($animation != "") ? " anima":"")),'<div class="icon-box">','</div>'); ?>
...
<div class="advs-box advs-box-multiple <?php echo $css ?>" data-anima="scale-rotate" data-trigger="hover">
    <a href="<?php echo esc_url($item["link"]) ?>" class="img-box <?php hc_echo($animation,"img-",""); ?>">
        <img alt="" src="<?php echo hc_get_img($item["image"]); ?>" />
    </a>
    <div class="circle anima-rotate-20 anima">
...
<div class="grid-list <?php echo hc_get_component_classes($Y_NOW,$EXTRA); if($Y_NOW["column"] == "col-md-12") echo " one-row-list"; ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    } else echo "<div class='text-center' style='width: 100%;'>" . esc_attr__("No ","hc") . $Y_NOW["post_type_slug"] . esc_attr__(" found.","hc") . "</div>";
        ?>
    </div>
    <?php
    if ($count > 0) {
...
function hc_include_hc_pt_menu(&$Y_NOW,$EXTRA) {
    ...
    if ($Y_NOW["post_type_slug"] != 'post') {
        $arr = get_terms(array('taxonomy' => $Y_NOW["post_type_slug"] . '-cat','hide_empty' => false));
        for ($i = 0; $i < count($arr); $i++){
            $menu_html .= '<li ' . (($HC_PT_CATEGORY == $arr[$i]->slug) ? 'class="active"':'') . '><a href="' . esc_url(get_category_link($arr[$i]->term_id)) .'">' . esc_attr($arr[$i]->name) . '</a></li>';
        }
    }  else {
        $arr = get_categories();
        foreach ($arr as $term ) {
            if ($term->term_id > 1) $menu_html .= '<li ' . (($HC_PT_CATEGORY == $term->slug) ? 'class="active"':'') . '><a href="' . esc_url(get_category_link($term->term_id)) .'">' . esc_attr($term->name) . '</a></li>';
        }
    }
