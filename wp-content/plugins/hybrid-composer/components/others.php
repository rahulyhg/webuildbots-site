...
<ul <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="<?php echo $list_class . ' ' . (($style != "") ? 'no-icons':'') . ' ' . esc_attr($Y_NOW["icon_size"] . ' ' . $Y_NOW["alignment"]) . ' ' . hc_get_component_classes($Y_NOW,$EXTRA); ?>"
    ...
    for ($i = 0; $i < count($arr); $i++) {
        ...
        else echo '<li><i class="fa-li ' . esc_attr($arr[$i]["icon"]) . '"></i> ' . wp_kses($arr[$i]["text"],array('b' => array())) . '</li>';
    }
    ?>
</ul>
<?php }
...
<div class="list-items <?php echo hc_get_component_classes($Y_NOW,$EXTRA) . " " . $css; ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
        ...
        for ($i = 0; $i < count($arr); $i++) {
            ...
            $html .= '<div class="list-item' . (($isImage) ? " list-item-img":"" ) . '"><div class="row"><div class="col-md-9">' . $img_html . '<h3>' . esc_attr($arr[$i]["title"]) . '</h3><p>' . $arr[$i]["subtitle"] . '</p></div><div class="col-md-3"><span>' . $arr[$i]["extra"] . '</span></div></div></div>';
        }
        echo $html;
    ?>
</div>
...
?>
<ul class="list-texts <?php echo hc_get_component_classes($Y_NOW,$EXTRA) . " " . $css ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]) ?>">
    <?php
        $arr = $Y_NOW["rows"];
        $html = "";
        for ($i = 0; $i < count($arr); $i++) {
            $html .= ' <li><b>' . esc_attr($arr[$i]["title"]) . '</b> <span>' . $arr[$i]["subtitle"] . '</span>' . hc_get($arr[$i]["extra"],' <b>','</b>') . '</li>';
        }
        echo $html;
    ?>
</ul>
...
<div class="counter-box-icon icon-box <?php if ($Y_NOW["style"] == "icon") echo "icon-box-top-bottom "; echo hc_get_component_classes($Y_NOW,$EXTRA); ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <div class="icon-box-cell">
        <?php echo hc_get_icon($Y_NOW["icon"], $Y_NOW["icon_style"], $Y_NOW["icon_image"], "icon text-l"); ?>
    </div>
    <div class="icon-box-cell">
        <?php
        $before = hc_get_now($Y_NOW,"before_text");
...
function hc_include_hc_icon_links(&$Y_NOW,$EXTRA) {
    ...
    if ($Y_NOW["type"] == "button" || $Y_NOW["type"] == "button_2") {
        $html = '<div class="social-group-button ' . (($Y_NOW["type"] == "button_2") ? "bottom-icons":"") . '"><div class="social-button" data-anima="scale-up" data-trigger="hover">
...
<form action="<?php echo HC_PLUGIN_URL ?>/scripts/php/contact-form.php" class="form-box form-ajax form-ajax-wp <?php echo hc_get_component_classes($Y_NOW,$EXTRA); if ($Y_NOW["label"] == "true") echo " label-visible"; ?>" method="post" data-email="<?php echo $Y_NOW["recipient_email"]; ?>" <?php hc_echo(hc_get_now($Y_NOW,"subject"),'data-subject="','"') ?> <?php hc_echo(esc_attr($Y_NOW["custom_css_styles"]),'style="','"'); ?>>
    ...
    if ($Y_NOW["button_text"] != "") {
        ...
        if ($button == "link") {
            ...
            $button .= " " . hc_get_now($Y_NOW,"button_dimensions");
            if (hc_get_now($Y_NOW,"button_animation")) $button .= " anima-button ";
        }
        if ($Y_NOW["button_position"] == "inline") $button .=  " pull-right ";
        $button = '<button class="' . $button . '" type="submit">' . ((strlen($Y_NOW["button_icon"]) > 0) ? "<i class='" . $Y_NOW["button_icon"] . "'></i>":"") . esc_attr($Y_NOW["button_text"]) . '</button>';
    }
    ...
    for ($i = 0; $i < count($arr); $i++) {
        ...
        <?php }
    $loader = '<img class="cf-loader" src="' . HC_PLUGIN_URL . '/images/loader.svg" />';
...
function hc_include_hc_code_block(&$Y_NOW,$EXTRA) {
    ...
    if (ctype_xdigit($Y_NOW["content"])) {
        if ($Y_NOW["language"] == "js") echo "<script>" . hex2bin($Y_NOW["content"]) . "</script>";
        if ($Y_NOW["language"] == "css") echo "<style>" . hex2bin($Y_NOW["content"]) . "</style>";
        if ($Y_NOW["language"] == "") echo hex2bin($Y_NOW["content"]);
    }
    echo '</div>';
}
